...
api_key = 'AIzaSyBdHLurLZHmhqkPAoIY8GmkxDz7iYpKO2U' #create api key from "https://console.developers.google.com/"
youtube = build('youtube', 'v3', developerKey=api_key)
#to connect MongoDb
conn1=pymongo.MongoClient("mongodb+srv://niteesh:123654@cluster0.bdvpixl.mongodb.net/") #mongodb atlas->sign->database->connect->drivers ->Add your connection string into your application code copy the link and paste it here and give userid and password
...
conn=psycopg2.connect(host="localhost",
                      ...
                      password="root",
                      port=5432,
                      database="youtube") #go to sql tool create database use the same name in database and password to connect sql
cursor=conn.cursor()
        ...
        for i in response['items']:
            ...
            for comment in range(len(response2["items"])):
                try:
                    ...
                    Comment_PublishedAt = response2["items"][comment][
                ...
                for i in response['items']:
                    ...
                    for comment in range(len(response5["items"])):
                        try:
                            ...
                            comment_Author = response5["items"][comment]["snippet"][
                                "topLevelComment"]["snippet"]["authorDisplayName"],
                            Comment_PublishedAt = response5["items"][comment][
                                "snippet"]["topLevelComment"]["snippet"]["publishedAt"]
                            data = {f"comments_id_{comment+1}": {"Comment_id": Comment_id[0],
                                                                 "comment_text": comment_text, "comment_Author": comment_Author[0],
                                                                 "Comment_PublishedAt": Comment_PublishedAt}}
    ...
    with col3:    
        with st.container():        
            #with st.expander("Channel Details", expanded=True):           
                json_output = json.dumps(json_file[0], indent=4)
...
text = "YouTube Data"   
st.markdown(f"<h2 style='color: blue; text-align: center;'>{text} &#127909;</h2>", unsafe_allow_html=True)
st.markdown(f"""<style> p {background-image:url('sai.jpg');}</style>""", unsafe_allow_html=True)
...
col2.text("")
col2.text("")
search_button = col2.button(":blue[GET]:mag_right:") #this button created for passing channel_id as input 
...
mongodb = col6.button(":red[Import to MongoDB]")
